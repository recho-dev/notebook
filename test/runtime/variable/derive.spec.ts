import {Runtime, type Module, type ModuleDefinition} from "@/lib/runtime/index.ts";
import assert from "assert";
import {it} from "vitest";
import {identity} from "@/lib/runtime/utils.ts";
import {valueof, promiseInspector, sleep} from "../utils.ts";
import type {ObserverInput, Variable} from "@/lib/runtime/variable.ts";
...
it("module.derive(overrides, module) injects variables into a copied module", async () => {
  ...
  const c0 = module0.variable(true).define("c", ["a", "b"], (a: unknown, b: unknown) => (a as number) + (b as number));
  const module1 = runtime.module();
  const module1_0 = module0.derive([{name: "d", alias: "b"}], module1);
  const c1 = module1_0.variable(true).define(null, ["c"], (c: unknown) => c);
...
it("module.derive(…) copies module-specific builtins", async () => {
  ...
  const module0 = runtime.module();
  module0.builtin("a", 1);
  const b0 = module0.variable(true).define("b", ["a"], (a: unknown) => (a as number) + 1);
  const module1_0 = module0.derive([], module0);
  const c1 = module1_0.variable(true).define("c", ["a"], (a: unknown) => (a as number) + 2);
  assert.deepStrictEqual(await valueof(b0), {value: 2});
...
it("module.derive(…) can inject into modules that inject into modules", async () => {
  ...
  A.define("a", 1);
  A.define("b", 2);
  A.define("c", ["a", "b"], (a: unknown, b: unknown) => (a as number) + (b as number));
  ...
  assert.strictEqual(g.module, C);
  assert.strictEqual(g.inputs[0].module, CB);
  assert.strictEqual(g.inputs[0].inputs[0].module.source, BA);
  assert.strictEqual(C.source, null);
  assert.strictEqual(CB.source, B);
  assert.strictEqual(BA.source, A);
});
...
it("module.derive(…) can inject into modules that inject into modules that inject into modules", async () => {
  ...
  assert.strictEqual(i.module, D);
  assert.strictEqual(i.inputs[0].module, DC);
  assert.strictEqual(i.inputs[0].module.source, C);
  assert.strictEqual(i.inputs[0].inputs[0].module.source, CB);
  assert.strictEqual(i.inputs[0].inputs[0].module.source.source, B);
});
...
it("module.derive(…) does not copy non-injected modules", async () => {
  ...
  assert.strictEqual(g.module, C);
  assert.strictEqual(g.inputs[0].module, CB);
  assert.strictEqual(g.inputs[0].inputs[0].module, A);
});
...
it("module.derive() supports lazy import-with", async () => {
  let resolve2: ((value: ModuleDefinition) => void) | undefined;
  const promise2: Promise<ModuleDefinition> = new Promise((resolve) => (resolve2 = resolve));

  function define1(runtime: Runtime, observer: (name?: string) => ObserverInput) {
    const main = runtime.module();
    main.define("module 1", ["@variable"], async (v: Variable) =>
      runtime.module(await promise2).derive([{name: "b"}], v.module),
    );
    ...
    main.variable(observer("imported c")).define("imported c", ["c"], (c: unknown) => c);
    return main;
  }
  ...
  function define2(runtime: Runtime, observer: (name?: string) => ObserverInput) {
    ...
    main.variable(observer("c")).define("c", ["a", "b"], (a: unknown, b: unknown) => (a as number) + (b as number));
    return main;
  }
...
it("module.derive() supports lazy transitive import-with", async () => {
  ...
  let resolve3: ((value: ModuleDefinition) => void) | undefined;
  const promise3 = new Promise<ModuleDefinition>((resolve) => (resolve3 = resolve));
  let module2_1: Module;
  let module3_2: Module;
  let variableC_1: Variable;
  ...
  function define1(runtime: Runtime, observer: (name?: string) => ObserverInput) {
    ...
    main.define(
      "module 2",
      ["@variable"],
      async (v: Variable) => (module2_1 = runtime.module(await promise2).derive([{name: "b"}], v.module)),
    );
    variableC_1 = main.define("c", ["module 2", "@variable"], (_: Module, v: Variable) => v.import("c", _));
  ...
  function define2(runtime: Runtime, observer: (name?: string) => ObserverInput) {
    ...
    main.define(
      "module 3",
      ["@variable"],
      async (v: Variable) => (module3_2 = runtime.module(await promise3).derive([{name: "b"}], v.module)),
    );
    main.define("c", ["module 3", "@variable"], (_: unknown, v: unknown) =>
      (v as {import: (name: string, module: unknown) => unknown}).import("c", _),
    );
    main.variable(observer("b")).define("b", [], () => 3);
    main.variable(observer()).define(["c"], (c: unknown) => c);
    return main;
  }
  ...
  const c1 = module1.scope.get("c");
  assert.strictEqual(c1, variableC_1!);
  assert.deepStrictEqual(
    c1.inputs.map((i) => i.name),
    ["module 2", "@variable"],
  );
  assert.strictEqual(Reflect.get(runtime, "_modules").size, 1);
  ...
  assert.strictEqual(c1.definition, identity);
  assert.strictEqual(c1.inputs[0].module, module2_1!);
  assert.strictEqual(module2_1!.source, module2);
  assert.strictEqual(Reflect.get(runtime, "_modules").size, 2);
  const b2_1 = module2_1!.scope.get("b");
  assert.deepStrictEqual(
    b2_1!.inputs.map((i) => i.name),
    ["b"],
  );
  assert.deepStrictEqual(b2_1!.definition, identity);
  assert.deepStrictEqual(b2_1!.inputs[0].module, module1);
  ...
  const c2_1 = module2_1!.scope.get("c");
  assert.strictEqual(c2_1!.module, module2_1!);
  assert.strictEqual(c2_1!.definition, identity);
  assert.strictEqual(c2_1!.inputs[0].module, module3_2!);
  assert.strictEqual(module3_2!.source, module3);
  const b3_2 = module3_2!.scope.get("b");
  assert.deepStrictEqual(
    b3_2!.inputs.map((i) => i.name),
    ["b"],
  );
  assert.deepStrictEqual(b3_2!.definition, identity);
  assert.deepStrictEqual(b3_2!.inputs[0].module, module2_1!);
