...
import {sleep, valueof} from "../utils.ts";
...
it("variable.define(name, function) can define a named variable", async () => {
  ...
  const module = runtime.module();
  const foo = module.variable(true).define("foo", () => 42);
  const bar = module.variable(true).define("bar", ["foo"], (foo: unknown) => foo);
  assert.deepStrictEqual(await valueof(foo), {value: 42});
...
it("variable.define recomputes reachability as expected", async () => {
  ...
  const baz = module.define("baz", ["quux"], (quux: unknown) => `baz-${quux}`);
  const bar = module.define("bar", ["quux"], (quux: unknown) => `bar-${quux}`);
  const foo = main
    .variable(true)
    .define("foo", ["bar", "baz", "quux"], (bar: unknown, baz: unknown, quux: unknown) => [bar, baz, quux]);
  main.variable().import("bar", module);
  main.variable().import("baz", module);
  main.variable().import("quux", module);
  await runtime.compute();
  assert.strictEqual(quux.reachable, true);
  assert.strictEqual(baz.reachable, true);
  assert.strictEqual(bar.reachable, true);
  assert.strictEqual(foo.reachable, true);
  assert.deepStrictEqual(await valueof(foo), {value: ["bar-42", "baz-42", 42]});
  foo.define("foo", [], () => "foo");
  await runtime.compute();
  assert.strictEqual(quux.reachable, false);
  assert.strictEqual(baz.reachable, false);
  assert.strictEqual(bar.reachable, false);
  assert.strictEqual(foo.reachable, true);
...
it("variable.define correctly detects reachability for unreachable cycles", async () => {
  ...
  const main = runtime.module();
  const module = runtime.module();
  const bar = module.define("bar", ["baz"], (baz: unknown) => `bar-${baz}`);
  const baz = module.define("baz", ["quux"], (quux: unknown) => `baz-${quux}`);
  const quux = module.define("quux", ["zapp"], function* (zapp: unknown) {
    try {
      while (true) yield `quux-${zapp}`;
    } finally {
      returned = true;
    }
  });
  const zapp = module.define("zapp", ["bar"], (bar: unknown) => `zaap-${bar}`);
  await runtime.compute();
  assert.strictEqual(bar.reachable, false);
  assert.strictEqual(baz.reachable, false);
  assert.strictEqual(quux.reachable, false);
  assert.strictEqual(zapp.reachable, false);
  ...
  main.variable().import("bar", module);
  const foo = main.variable(true).define("foo", ["bar"], (bar: unknown) => bar);
  await runtime.compute();
  assert.strictEqual(foo.reachable, true);
  assert.strictEqual(bar.reachable, true);
  assert.strictEqual(baz.reachable, true);
  assert.strictEqual(quux.reachable, true);
  assert.strictEqual(zapp.reachable, true);
...
it("variable.define does not terminate reachable generators", async () => {
  ...
  const baz = main.variable(true).define("baz", ["bar"], (bar: unknown) => bar);
  const foo = main.variable(true).define("foo", ["bar"], (bar: unknown) => bar);
  main.variable().import("bar", module);
  assert.deepStrictEqual(await valueof(foo), {value: 1});
...
it("variable.define detects duplicate declarations", async () => {
  ...
  const v3 = main.variable(true).define(null, ["foo"], (foo: unknown) => foo);
...
it("variable.define allows masking of builtins", async () => {
  ...
  const foo = main.variable(true).define(null, ["color"], (color: unknown) => color);
...
it("variable.define supports generator cells", async () => {
  ...
  const runtime = new Runtime();
  const main = runtime.module();
  const foo = main.variable(true).define("foo", [], function* () {
    while (i < 3) yield ++i;
  });
  assert.deepStrictEqual(await valueof(foo), {value: 1});
...
it("variable.define supports generators that yield promises", async () => {
  ...
  const main = runtime.module();
  const foo = main.variable(true).define("foo", [], function* () {
    while (i < 3) yield Promise.resolve(++i);
  });
  assert.deepStrictEqual(await valueof(foo), {value: 1});
  assert.deepStrictEqual(await valueof(foo), {value: 2});
  assert.deepStrictEqual(await valueof(foo), {value: 3});
...
it("variable.define recomputes downstream values when a variable is renamed", async () => {
  ...
  const baz = main
    .variable(true)
    .define("baz", ["foo", "bar"], (foo: unknown, bar: unknown) => (foo as number) + (bar as number));
  assert.deepStrictEqual(await valueof(foo), {value: 1});
  assert.deepStrictEqual(await valueof(bar), {value: 2});
...
it("variable.define ignores an asynchronous result from a redefined variable", async () => {
  ...
  const foo = main
    .variable(true)
    .define("foo", [], () => new Promise((resolve) => setTimeout(() => resolve("fail"), 150)));
  await new Promise(setImmediate);
  foo.define("foo", [], () => "success");
  await new Promise((resolve) => setTimeout(resolve, 250));
  assert.deepStrictEqual(await valueof(foo), {value: "success"});
  assert.deepStrictEqual(foo.value, "success");
});
...
it("variable.define does not try to compute unreachable variables", async () => {
  ...
  let evaluated = false;
  const foo = main.variable(true).define("foo", [], () => 1);
  const bar = main.variable().define("bar", ["foo"], (foo: unknown) => (evaluated = foo as boolean));
  assert.deepStrictEqual(await valueof(foo), {value: 1});
  assert.deepStrictEqual(await valueof(bar), {value: undefined});
...
it("variable.define does not try to compute unreachable variables that are outputs of reachable variables", async () => {
  ...
  const baz = main
    ...
    .define(
      "baz",
      ["foo", "bar"],
      (foo: unknown, bar: unknown) => (evaluated = ((foo as number) + (bar as number)) as unknown as boolean),
    );
  assert.deepStrictEqual(await valueof(foo), {value: 1});
  assert.deepStrictEqual(await valueof(bar), {value: 2});
  assert.deepStrictEqual(await valueof(baz), {value: undefined});
...
it("variable.define can reference whitelisted globals", async () => {
  const runtime = new Runtime(null, (name: string) => (name === "magic" ? 21 : undefined));
  const module = runtime.module();
  const foo = module.variable(true).define(["magic"], (magic: unknown) => (magic as number) * 2);
  assert.deepStrictEqual(await valueof(foo), {value: 42});
});

it("variable.define captures the value of whitelisted globals", async () => {
  let magic = 0;
  const runtime = new Runtime(null, (name: string) => (name === "magic" ? ++magic : undefined));
  const module = runtime.module();
  const foo = module.variable(true).define(["magic"], (magic: unknown) => (magic as number) * 2);
  assert.deepStrictEqual(await valueof(foo), {value: 2});
  assert.deepStrictEqual(await valueof(foo), {value: 2});
...
it("variable.define can override whitelisted globals", async () => {
  const runtime = new Runtime(null, (name: string) => (name === "magic" ? 1 : undefined));
  const module = runtime.module();
...
it("variable.define correctly handles globals that throw", async () => {
  const runtime = new Runtime(null, (name: string) => {
    if (name === "oops") throw new Error("oops");
  });
  const module = runtime.module();
  const foo = module.variable(true).define(["oops"], (oops: unknown) => oops);
...
it("variable.define allows other variables to begin computation before a generator may resume", async () => {
  ...
  let genIteration = 0;
  let valIteration = 0;
  const onGenFulfilled = (value: unknown) => {
    if (genIteration === 0) {
  ...
  const onValFulfilled = (value: unknown) => {
    if (valIteration === 0) {
  ...
  const val = main.variable({fulfilled: onValFulfilled}).define("val", ["gen"], (i: unknown) => i);
  assert.strictEqual(await gen.promise, undefined, "gen cell undefined");
  assert.strictEqual(await val.promise, undefined, "val cell undefined");
  await runtime.compute();
  assert.strictEqual(await gen.promise, 1, "gen cell 1");
  assert.strictEqual(await val.promise, 1, "val cell 1");
  await runtime.compute();
  assert.strictEqual(await gen.promise, 2, "gen cell 2");
  assert.strictEqual(await val.promise, 2, "val cell 2");
  await runtime.compute();
  assert.strictEqual(await gen.promise, 3, "gen cell 3");
  assert.strictEqual(await val.promise, 3, "val cell 3");
});
...
it("variable.define allows other variables to begin computation before a generator may resume", async () => {
  ...
  const val = main.variable(true).define("val", ["gen"], (gen: unknown) => {
    j++;
    assert.strictEqual(gen, j, "gen = j");
    assert.strictEqual(gen, i, "gen = i");
    return gen;
  });
  assert.strictEqual(await gen.promise, undefined, "gen = undefined");
  assert.strictEqual(await val.promise, undefined, "val = undefined");
  await runtime.compute();
  assert.strictEqual(await gen.promise, 1, "gen cell 1");
  assert.strictEqual(await val.promise, 1, "val cell 1");
  await runtime.compute();
...
it("variable.define does not report stale fulfillments", async () => {
  ...
  await Reflect.get(variable, "_computing");
...
it("variable.define waits for the previous value to settle before computing", async () => {
  ...
  const main = runtime.module();
  const log: string[] = [];
  let resolve1: ((value: unknown) => void) | undefined;
  let resolve2: ((value: unknown) => void) | undefined;
...
it("variable.define skips stale definitions", async () => {
  ...
  let resolve1: ((value: unknown) => void) | undefined;
  let resolve2: ((value: unknown) => void) | undefined;
  let resolve3: ((value: unknown) => void) | undefined;
  const promise1 = new Promise((r) => (resolve1 = r));
  const promise2 = new Promise((r) => (resolve2 = r));
